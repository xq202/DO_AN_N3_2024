...
public interface TicketRepository extends JpaRepository<TicketEntity, Integer> {
    ...
    Page<TicketEntity> findByCarCodeContainingIgnoreCaseAndEndDateAfterAndCreatedAtBetweenAndCarUserId(String carCode, Timestamp now, Timestamp startDate, Timestamp endDate, int userId, Pageable pageable);

    @Query("SELECT t FROM TicketEntity t WHERE t.car.code like concat('%', ?1, '%') AND t.car.user.email like concat('%', ?2, '%') AND (t.ticketType.id = ?3 OR ?3 = 0) AND t.createdAt between ?4 and ?5 and ((?6 = 1 and t.endDate < now()) or (?6 = 0 and t.endDate >= now()) or ?6 = -1)")
